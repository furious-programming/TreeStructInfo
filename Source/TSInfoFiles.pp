...
var
  ...
  LNodePath, LNodeNameAsPath: String;
  LPathIsSymbol: Boolean;
begin
  ...
  begin
    LNodePath := ANodePath;
    LPathIsSymbol := IsCurrentNodePath(LNodePath);

    if LPathIsSymbol then
      LParentNode := FCurrentNode
    else
    begin
      LNodePath := IncludeTrailingIdentsDelimiter(ANodePath);
      LParentNode := FindNode(LNodePath, True);
    end;
    ...
    begin
      ...
      if AOpen then
      begin
        LNodeNameAsPath := IncludeTrailingIdentsDelimiter(ANodeName);

        if FCurrentNode = FRootNode then
        begin
          if LPathIsSymbol then
            FCurrentlyOpenNodePath := LNodeNameAsPath
          else
            FCurrentlyOpenNodePath := LNodePath + LNodeNameAsPath;
        end
        else
          if LPathIsSymbol then
            FCurrentlyOpenNodePath += LNodeNameAsPath
...
var
  LNameBegin, LNameEnd: PChar;
begin
  LNameBegin := @ALine[KEYWORD_REF_NODE_LEN] + 1;
  LNameEnd := @ALine[Length(ALine)];
...
var
  ...
  LNodeIdx: Integer;
begin
  for LNodeIdx := 0 to AParentNode.ChildNodesCount - 1 do
  begin
    LNode := AParentNode.GetChildNode(LNodeIdx);

    if LNode.Reference then
    begin
      AddRefChildNodeDeclaration(LNode);
      FStoreRefElement(LNode);
    end
    else
      AddStdChildNode(LNode);
  end;
end;
